...
def fitting(experimentdata=dict,simulationdata=dict,strainl=list):
    ...
    for strain in strainl_experiment:
        ...
        #loss_sample = loss(experimentdata[strain],stress_simulation)
    return stress_simulations
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        for sample in sample_list:
            ...
            stress_simulations = fitting(sample_data[sample_list.index(sample)],stran_stress_dict,strainl)

            import csv

            # result 为插入的list数据
            dataframe = pd.DataFrame({'n': [int(data[0]) for i in range(len(stress_simulations))]
                                    , 'h0': [int(data[1]) for i in range(len(stress_simulations))]
                                    , "s0":[int(data[2]) for i in range(len(stress_simulations))]
                                    ,"si":[int(data[3]) for i in range(len(stress_simulations))]
                                    ,"stain":list(sample_data[sample_list.index(sample)].keys())
                                    ,"sr":stress_simulations
                                    ,"ex":list(sample_data[sample_list.index(sample)].values())})

            # 'Experiments.csv' 为导出的文件名
            dataframe.to_csv('data_full.csv', mode='a', header=False,index=False)
